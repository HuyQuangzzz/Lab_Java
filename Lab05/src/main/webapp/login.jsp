...
<c:if test="${not empty sessionScope.username}">
    <c:redirect url="${pageContext.request.contextPath}/product.jsp" />
</c:if>
<c:if test="${not empty cookie.email}">
    <c:set var="rememberedEmail" value="${cookie.email.value}" />
</c:if>
<c:if test="${not empty cookie.password}">
    <c:set var="rememberedPassword" value="${cookie.password.value}" />
</c:if>
<div class="container">
