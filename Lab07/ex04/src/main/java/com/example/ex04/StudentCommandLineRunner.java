...
import com.example.ex04.repositories.StudentRepository;
...
public class StudentCommandLineRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        // Đọc danh sách học sinh có tuổi lớn hơn hoặc bằng x,
        Iterable<Student> studentage = studentRepository.findByAgeGreaterThan(22);
        System.out.println("Danh sách học sinh có tuổi lớn hơn hoặc bằng x:");
        studentage.forEach(student -> System.out.println(student.toString()));

        // Đếm số học sinh có điểm ielts bằng x
        System.out.println("Số sinh viên có điểm bằng x: "+ studentRepository.countByIelts(6.0) +" sinh viên");


        // Danh sách sinh viên có tên chứa từ xxx
        Iterable<Student> studentName = studentRepository.findByNameContaining("A");
        System.out.println("Danh sách sinh viên có tên chứa từ xxx: ");
        studentName.forEach(student -> System.out.println(student.toString()));
    }
}